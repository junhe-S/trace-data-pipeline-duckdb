...
def _trace_chunk_worker(args: dict) -> tuple[list, list, list]:
    ...
    logging.info(f"Processing chunk {i} of {n_total}")

    # ── Fetch via local DuckDB ───────────────────────────────────────────────
    sql = f"""
    ...
    if start_date:
        sql += " AND trd_exctn_dt >= ?"
        params.append(start_date)

    local_db = duckdb.connect(database=db_path, read_only=True)
    trace = local_db.execute(sql, params).df()
    local_db.close()

    logging.info(f"Chunk {i}: Retrieved {len(trace)} rows")
    ...
    if f["dick_nielsen"]:
        clean_chunk = clean_trace_standard_chunk(trace, chunk_id=i, logger=log_ct_filter)
    ...
    if f["decimal_shift_corrector"]:
        ...
        trace, n_rows_replaced, replace_cusips = decimal_shift_corrector(
            trace.sort_values(["cusip_id", "trd_exctn_dt", "trd_exctn_tm"]), **_ds
        )
        if replace_cusips:
    ...
    if f["trading_calendar"]:
        ...
        trace = filter_by_calendar(df=trace, calendar_name=calendar_name,
                                    date_col="trd_exctn_dt", start_date=start_date,
    ...
    if f["volume_filter_toggle"]:
        ...
        if vkind == "dollar":
            mask, stage_name = trace["dollar_vol"] >= vthr, "volume_filter[dollar]"
        else:
            mask, stage_name = trace["entrd_vol_qt"] >= vthr, "volume_filter[par]"
    ...
    if f["bounce_back_filter"]:
        ...
        trace = flag_price_change_errors_nb(
            trace.sort_values(["cusip_id", "trd_exctn_dt", "trd_exctn_tm"]), **_bb
        )
        bb_cusips = list(trace[trace["filtered_error"] == 1]["cusip_id"].unique())
        if bb_cusips:
            bb_cusips_chunk.extend([str(c) for c in bb_cusips])
    ...
    logging.info(f"Chunk {i}: wrote {out_path} ({elapsed}s)")
...
def clean_trace_data(
    ...
    *,
    ...
    chunk_args = [
        dict(
            i                 = i,
            cusip_list        = list(cusip_chunks[i - 1]),
            fisd_off          = fisd_off,
            f                 = f,
    ...
    _bulk_load_parquet_to_duckdb(temp_dir, output_db_path, table_name=f"trace_{data_type}")                

    return bb_cusips_all, dec_shift_cusips_all, init_price_cusips_all


# -------------------------------------------------------------------------
def _bulk_load_parquet_to_duckdb(temp_dir: str, output_db_path: str, table_name: str = "trace_standard"):
...
# -------------------------------------------------------------------------
def build_fisd(params: dict | None = None, *, data_type: str = "standard"):
    """
